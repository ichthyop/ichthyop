...
public class EvolManager extends AbstractManager implements SetupListener {
    ...
    public void cfgEvolGenToCfgIchthyop() {
        ...
        parent = parent.concat(System.getProperty("file.separator")).concat(getSimulationManager().getId()).concat("_Release");
    ...
    private void cfgEvolToCfgIchthyop() {
         ...
         * Mycfg.addBlock(cfgTemplate.getBlock(BlockType.RELEASE,
         * "release.uniform").detach()); Mycfg.getBlock(BlockType.RELEASE,
         * "release.uniform").setEnabled(true); String nb_particles =
         * Mycfg.getXParameter(BlockType.OPTION, "release.evol",
         * "nb_particles").getValue(); String depth_min =
         * Mycfg.getXParameter(BlockType.OPTION, "release.evol",
         * "depth_min").getValue(); String depth_max =
         * Mycfg.getXParameter(BlockType.OPTION, "release.evol",
         * "depth_max").getValue();
         *
         * Mycfg.getXParameter(BlockType.RELEASE, "release.uniform",
         * "number_particles").setValue(nb_particles);
         * Mycfg.getXParameter(BlockType.RELEASE, "release.uniform",
         * "depth_max").setValue(depth_max);
         * Mycfg.getXParameter(BlockType.RELEASE, "release.uniform",
    ...
    public double createMarginLon(double x, double marge, double lat) {
    ...
    public double createMarginLat(double x, double marge) {
    ...
    public double createTimeMargin(double time, double margin, int count) {
        ...
        newYear.append(" month 01 day 01 at 00:00");
        System.out.println("***************** newYear= " + newYear.toString());
        double maxTime = 0.d;
        try {
            maxTime = getSimulationManager().getTimeManager().date2seconds(newYear.toString());
            //System.out.println("--------------------- maxTime: " + maxTime+ " et newTime = "+newTime);
        } catch (ParseException ex) {
            Logger.getLogger(EvolManager.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        if ((newTime >= maxTime) || (newTime < getSimulationManager().getTimeManager().get_tO())) {
            return createTimeMargin(time, margin, count);
        }
        return newTime;
    }
    ...
    public double[][] createArrayNewParticles(double[][] candidates, List<Integer> selectedList, float time_marge, float marge, float bathy_marge) {
        ...
        for (int i = 0; i < newParticles.length; i++) {
            newParticles[i][0] = Math.abs(createTimeMargin(candidates[selectedList.get(i)][0], time_marge, 20));
            ...
            //System.out.println("++++++++ Lat_PONTE = " + candidates[selectedList.get(i)][2] + " Lat_PONTE_EPSILON " + newParticles[i][2]);
            newParticles[i][1] = createMarginLon(candidates[selectedList.get(i)][1], marge, newParticles[i][2]);
            //System.out.println("++++++++ Lon_PONTE = " + candidates[selectedList.get(i)][1] + " Lon_PONTE_EPSILON " + newParticles[i][1]);
            newParticles[i][3] = createMarge(candidates[selectedList.get(i)][3], bathy_marge);
            //System.out.println("++++++++ T_PONTE = " + candidates[selectedList.get(i)][3] + " Bath_PONTE_EPSILON " + newParticles[i][3]);
            int limit = 20;
            boolean exit_ = false;
            boolean isParticleInWater = false;
            boolean isParticleOnEdge = true;
            boolean isParticleUnderLevelZero = true;
            double x, y, z;
            do {
                double[] pGrid = getSimulationManager().getDataset().lonlat2xy(newParticles[i][1], newParticles[i][2]);
                isParticleInWater = getSimulationManager().getDataset().isInWater(pGrid);
                isParticleOnEdge = getSimulationManager().getDataset().isOnEdge(pGrid);
                x = pGrid[0];
                y = pGrid[1];
                isParticleUnderLevelZero = Math.abs(newParticles[i][3]) > Math.abs(getSimulationManager().getDataset().z2depth(x, y, 0));
                //z = getSimulationManager().getDataset().depth2z(x, y, newParticles[i][3]);
            ...
            //System.out.println("En sortie de la boucle:exit_ =  " + exit_ + "--  isParticleInWater= " + isParticleInWater + " --isParticleOnEdge=" + isParticleOnEdge);
            if (limit == 0) {
                System.out.println("limit = 0 => récupérer caractéristiques des parents");
                newParticles[i][2] = candidates[selectedList.get(i)][2];
                newParticles[i][1] = candidates[selectedList.get(i)][1];
            }
        }
        return newParticles;
    }
    ...
    public void createReleaseNextGeneration(double[][] parentParticles, double[][] newParticles, int selectedYear) throws IOException {
        String nomFile = getSimulationManager().getOutputManager().getFileLocation();
        String parent = new File(nomFile).getParent();
        parent = parent.concat(System.getProperty("file.separator"));
        parent = parent.concat(getSimulationManager().getId()).concat("_Release");
        ...
        if (newParticles != null) {
            for (int i = 0; i < newParticles.length; i++) {
                ...
                //calendar.set(Calendar.YEAR, calendar.get(Calendar.YEAR) + 0);
                calendar.set(Calendar.YEAR, selectedYear);
                //System.out.println("============= le calendrier est à l'année: "+calendar.get(calendar.YEAR));
                String timeAsStr = INPUT_DATE_FORMAT.format(calendar.getTime());
                //System.out.println("============= timeAsStr: "+timeAsStr);
