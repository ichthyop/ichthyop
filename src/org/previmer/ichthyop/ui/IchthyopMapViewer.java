 ...
 * @author pverley
 */
public class IchthyopMapViewer extends JMapViewer {
    
    private TileSource tileFactory =  new OsmTileSource.Mapnik();
    private List<Coordinate> region;
    private HashMap<String, IchthyopMapViewer.DrawableZone> zones;
    ...
    public void setWMS(TileSource tile) {
        setTileSource(tile);
        setZoom(defaultZoom);
    }
    ...
    public String[] getVariableList() {
        ...
        for (Variable variable : nc.getVariables()) {
            ...
            if (variable.getName().matches("release_zone")) {
                list.add(variable.getName());
            }
        }
    ...
    public List<Coordinate> getRegion() {
        if (null == region) {
            region = readRegion();
        }
        return region;
    ...
    HashMap<String, IchthyopMapViewer.DrawableZone> getZones() {
        if (null == zones) {
            zones = readZones();
        }
        return zones;
    ...
    private List<Coordinate> readRegion() {
        final List<Coordinate> lregion = new ArrayList();
        try {
            ArrayFloat.D2 regionEdge = (ArrayFloat.D2) nc.findVariable("region_edge").read();
            for (int i = 0; i < regionEdge.getShape()[0]; i++) {
                lregion.add(new Coordinate(regionEdge.get(i, 1), regionEdge.get(i, 0)));
            }
        } catch (IOException ex) {
    ...
    private HashMap<String, IchthyopMapViewer.DrawableZone> readZones() {
        HashMap<String, IchthyopMapViewer.DrawableZone> lzones = new HashMap();
        if (null != nc.findGlobalAttribute("nb_zones")) {
            ...
            for (int iZone = 0; iZone < nbZones; iZone++) {
                ...
                try {
                    ...
                    ArrayFloat.D2 zoneEdge = (ArrayFloat.D2) varZone.read();
                    ...
                    IchthyopMapViewer.DrawableZone zone = new IchthyopMapViewer.DrawableZone(edge, color);
    ...
    private void drawZone(IchthyopMapViewer.DrawableZone zone, Graphics2D g, JXMapViewer map) {
    ...
    private void drawZones(Graphics2D g, JXMapViewer map) {
        for (IchthyopMapViewer.DrawableZone zone : getZones().values()) {
            drawZone(zone, g, map);
        }
    }
    ...
    private void drawRegion() {
        addMapPolygon(new MapPolygonImpl(getRegion(), new Color(255, 255, 205, 50), new BasicStroke(2)));
    }
    ...
    private Painter<JMapViewer> getColorbarPainter() {

        Painter<JMapViewer> clrbarPainter = new Painter<JMapViewer>() {

            @Override
            public void paint(Graphics2D g, JMapViewer map, int w, int h) {
                g = (Graphics2D) g.create();
    ...
    private double readTime(int index) {
        double timeD = 0.d;
        try {
            ArrayDouble.D0 arrTime = (ArrayDouble.D0) vtime.read(new int[]{index}, new int[]{1}).reduce();
            timeD = arrTime.get();
        } catch (Exception ex) {
            Logger.getLogger(WMSMapper.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    private List<IchthyopMapViewer.DrawableParticle> getParticles(int index) {
        List<IchthyopMapViewer.DrawableParticle> list = new ArrayList();
        try {
            ...
            if (null != pcolorVariable) {
                ...
                } else if (pcolorVariable.getName().matches("release_zone")) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (arrMortality.get(i) == 0) {
                    if (null != arrColorVariable) {
                        if (pcolorVariable.getName().matches("time")) {
                            list.add(new IchthyopMapViewer.DrawableParticle(lon, arrLat.get(i), arrColorVariable.getFloat(0)));
                        } else if (pcolorVariable.getName().matches("release_zone")) {
                            Color colorZone = zones.get("release_zone" + arrColorVariable.getInt(i)).getColor();
                            list.add(new IchthyopMapViewer.DrawableParticle(lon, arrLat.get(i), colorZone));
                        } else {
                            list.add(new IchthyopMapViewer.DrawableParticle(lon, arrLat.get(i), arrColorVariable.getFloat(i)));
                        }
                        ...
                        list.add(new IchthyopMapViewer.DrawableParticle(lon, arrLat.get(i), Float.NaN));
                    }
                } else {
                    list.add(new IchthyopMapViewer.DrawableParticle(lon, arrLat.get(i)));
                }
            }
        } catch (Exception ex) {
            Logger.getLogger(WMSMapper.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public void writeKMLStep(int i) {
        ...
        for (IchthyopMapViewer.DrawableParticle particle : getParticles(i)) {
    ...
    private String createStyle(IchthyopMapViewer.DrawableParticle particle) {
    ...
    public void screen2File(final int index) {
        ...
        paintAll(g);
        new Thread(new IchthyopMapViewer.ImageWriter(index, bi)).start();
    }
    ...
    class DrawableParticle extends GeoPosition {
        ...
        DrawableParticle(float lon, float lat, Color color) {
            super(lat, lon);
            this.color = color;
