...
public class InitialSpawn {
   ...
   // static int excess;
    ...
    static long last_spawn = SimulationManager.getInstance().getTimeManager().get_tO();
    ...
    private boolean spawnDay(long last_spawn){
        ...
        long t0 = SimulationManager.getInstance().getTimeManager().getTime(); 
        long x = last_spawn + 172800;
        // ponte tous ls 2 jours = 172800 secondes
        if ((current_time == x)&& (x <= t0 + 31536000)){
            last_spawn += 172800;
            return true;
        }    
        else return false;        
