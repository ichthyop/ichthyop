 ...
 * @author pverley
 */
public class Regular2DProjected extends AbstractDataset {
...
////////////////////////////
    ...
    private void readConstantField() throws Exception {
        ...
        // scale factors
        dyv = (latitude[1] - latitude[0]);
        dxu = (longitude[1] - longitude[0]);
    ...
    void setAllFieldsTp1AtTime(int rank) throws Exception {
        ...
        for (int i = 0; i < variable.getShape()[0]; i++) {
            for (int j = 0; j < variable.getShape()[1]; j++) {
                index.set(i, j);
                u_tp1[i][j] = this.conversionFactor * variable.getDouble(index);
            }
        }
        ...
        for (int i = 0; i < variable.getShape()[0]; i++) {
            for (int j = 0; j < variable.getShape()[1]; j++) {
                index.set(i, j);
                v_tp1[i][j] = this.conversionFactor * variable.getDouble(index);
            }
        }
        ...
        for (RequiredVariable reqVar : requiredVariables.values()) {
            reqVar.nextStep(readVariable(ncU, reqVar.getName(), rank), time_tp1, dt_HyMo);
        }
    ...
    public double[] latlon2xy(double lat, double lon) {
        ...
        while (!(found1 && found2)) {
            int i = ci;
            int j = cj;
            double dmin = DatasetUtil.euclidianDistance(lat, lon, latitude[j], longitude[i]);
            for (int ii = -di; ii <= di; ii += di) {
                if ((i + ii >= 0) && (i + ii < nx)) {
                    double d = DatasetUtil.euclidianDistance(lat, lon, latitude[j], longitude[i + ii]);
                    if (d < dmin) {
                        dmin = d;
                        ci = i + ii;
                        cj = j;
            ...
            for (int jj = -dj; jj <= dj; jj += dj) {
                if ((j + jj >= 0) && (j + jj < ny)) {
                    double d = DatasetUtil.euclidianDistance(lat, lon, latitude[j + jj], longitude[i]);
                    if (d < dmin) {
                        dmin = d;
                        ci = i;
                        cj = j + jj;
        ...
        int cip1 = ci + 1 > nx - 1 ? nx - 1 : ci + 1;
        int cim1 = ci - 1 < 0 ? 0 : ci - 1;
        ...
        if (lon >= longitude[ci]) {
            ...
            //toto = String.format("ci = %d, dx=%f, deltax=%f, lon=%f, lonci=%f", ci, deltax, dx, lon, longitude[ci]);
            //System.out.println(toto);
        } else {
            double dx = longitude[ci] - longitude[cim1];
