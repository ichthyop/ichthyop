...
///////////////
    ...
    public Particle(int index,
        ...
        while (outZone) {
            ...
            outZone
                    = !Dataset.isInWater(this)
                      || (numReleaseZone == -1)
                      || isOnEdge(Dataset.get_nx(),
                                  Dataset.get_ny());

            if (is3D && !outZone) {
                depth = depthMin + Math.random() * (depthMax - depthMin);
                outZone = depth < data.getDepth(0);
            }
...
////////////////////////////
    ...
    void init() {
        ...
        if (FLAG_MIGRATION) {
            FLAG_ISODEPTH = (Simulation.getDepthDay()
        ...
        if ((numCurrentZone != -1) && !isRecruited[numCurrentZone]) {
            ...
            if (Configuration.isDepthRecruitment()) {
                satisfyCriterion = satisfyCriterion &&
                        (Math.abs(getDepth()) <= Simulation.getMaxDepthRecruitment()
                        && Math.abs(getDepth()) >= Simulation.getMinDepthRecruitment());
            }
            if (satisfyCriterion) {
...
//////////
    ...
    public double getKz(double time) {
        return data.getKv(getPGrid(), time, dt)[1];
    }
