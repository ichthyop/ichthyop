...
////////////////////////////
    ...
    public void init() throws Exception {
        ...
        if (Configuration.getTypeRelease() == Constant.RELEASE_ZONE) {
            depthReleaseMin = -1.f
                    * Configuration.getDepthReleaseMin(SerialParameter.
                    RELASE_DEPTH.index());
            depthReleaseMax = -1.f
                    * Configuration.getDepthReleaseMax(SerialParameter.
                    RELASE_DEPTH.index());
            if (Configuration.isPatchiness()) {
                patchRadius = Configuration.getRadiusPatchi(SerialParameter.
                        PATCHINESS.index());
                patchThickness = Configuration.getThickPatchi(SerialParameter.
                        PATCHINESS.index());
            }
        }
        ...
        if (Configuration.isLethalTp()) {
            lethalTpEgg = Configuration.getLethalTpEgg(SerialParameter.
                    LETHAL_TP_EGG.index());
            lethalTpLarvae = Configuration.getLethalTpLarvae(SerialParameter.
        ...
        if (Configuration.isBuoyancy()) {
            eggDensity = Configuration.getEggDensity(SerialParameter.BUOYANCY.
                    index());
            BuoyancyScheme.init();
        }
        ...
        } else if (Configuration.getTypeRecruitment() == Constant.RECRUIT_AGE) {
     ...
     */
    public void step(long time) throws IOException, NullPointerException {
    ...
    public void printParameters(Step step) {
        ...
        if (Configuration.isLethalTp()) {
            ...
            if (Configuration.isGrowth()) {
                console.append("--> Tp for egg [celsius]:");
        ...
        if (Configuration.getTypeRecruitment() != Constant.NONE) {
            ...
            case Constant.RECRUIT_AGE:
                console.append("Age min at recruitment [day]: ");
                console.append(ageMinAtRecruitment);
                break;
            case Constant.RECRUIT_LENGTH:
                console.append("Length min at recruitment [millimeter]: ");
