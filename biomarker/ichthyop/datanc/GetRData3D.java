...
//------------------------------------------------------------------------------
  ...
  void setFieldsName() {
    ...
    strBiomarker1 = GetConfig.getStrBiomarker1();
    strBiomarker2 = GetConfig.getStrBiomarker2();
  }
  ...
  void getCstSigLevels() {
    ...
    Attribute thetasA, thetabA, hcA;

    //-----------------------------------------------------------
    // Read the Param in ncIn
    thetasA = ncIn.getAttribute(strThetaS);
    thetas = (thetasA.getNumericValue()).doubleValue();
    thetabA = ncIn.getAttribute(strThetaB);
    thetab = (thetabA.getNumericValue()).doubleValue();
    hcA = ncIn.getAttribute(strHc);
    hc = (hcA.getNumericValue()).doubleValue();
  ...
  public void setAllFieldsAtTime(long time) {
    ...
    try {
      if (firstDay) {
        ...
        open(getFile(t0 / Resources.ONE_SECOND));
        timeTp1 = t0;
        setAllFieldsTp1AtTime(currentNcInRank = findCurrentRank(t0 / Resources.ONE_SECOND));
        firstDay = false;
      }
      ...
      if (currentNcInRank > (nbTimeRecords - 1)) {
        open(getNextFile(time / Resources.ONE_SECOND));
        currentNcInRank = 0;
      }
  ...
  void setAllFieldsTp1AtTime(int i_time) {
    ...
    if (GetConfig.isBiomarker1()) {
      try {
        biomarker1_tp1 = ncIn.get(strBiomarker1).copyout(new int[] {i_time, 0, 0, 0},
            new int[] {1, nz, ny, nx});
      }
      catch (Exception ex) {
        System.err.println(
            "!! Error --> Exception (ignored) - Problem extracting biomarker1 fields from file "
            + ncIn.getFile().toString());
      }
    }
    if (GetConfig.isBiomarker2()) {
      try {
        biomarker2_tp1 = ncIn.get(strBiomarker2).copyout(new int[] {i_time, 0, 0, 0},
            new int[] {1, nz, ny, nx});
      }
      catch (Exception ex) {
        System.out.println(
            "!! Error --> Exception (ignored) - Problem extracting biomarker2 fields from file "
            + ncIn.getFile().toString());
      }
    }


    dt_HyMo = Math.abs(timeTp1 - timeTp0);
    getSigLevelsTp1(1);
    xwTp1 = computeW();
