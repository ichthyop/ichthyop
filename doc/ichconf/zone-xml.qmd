...
- `type` specifies whether the zone should be used for release (see @sec-zone-release) or recruitment processes (`recruitment` value)
...
- `proportion_particles` specifies the proportion (values in $[0-1]$) of particles to be released in the area. Only used when `type` is `release` and if the `user_defined_nparticles` parameter is set to True (cf. @sec-zone-release)
