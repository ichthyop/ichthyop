...
Ichthyop simulations are configured using [XML](https://en.wikipedia.org/wiki/XML) configuration files. These files should always start as follows:
...
- `ACTION`: Block of parameters related to the `action` classes (cf. @sec-process).
- `RELEASE`: Block of parameters related to the `release` classes (cf. @sec-sec-release)
...
The `<enabled>` tag specifies whether the block should be considered when running the Ichthyop code or not. By default, all blocks are enabled.
But for the `RELEASE` and `DATASET` blocks, one and only one block must be enabled.
...
The `<key>` tag allows to identify the parameter, while the `<value>` tag specifies the value of this parameter.
The `<default>` tag provides the default value of the parameter.  The `<long_name>` and `<description>` tags are used by the console
...
providing as many `<accepted>` tags as necessary. For example:
...
If a parameter should by default appears as hidden in the Ichthyop console, it can be specified by adding the
...
When simulations are run with serial parameters, all possible combinations of parameter values will be run. Output files
will then contain a `_sX` suffix, with `X` the simulation number. The parameters used in a particular simulation `X` are provided as
global NetCDF attributes in the corresponding `_sX` NetCDF output file.
