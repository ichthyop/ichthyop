...
where $\Delta \lambda_{0}$ and $\Delta \phi_{0} $ are the changes in longitudes and latitudes due to wind, $\Delta \lambda$ and $\Delta \phi $ are the effective changes in longitudes and latitudes of the particle when taking into account the angle $\theta$, $\Delta t$ is Ichthyop time-step (in days) and $U_w$ and $V_w$ are the zonal and meridional wind components (in $m.s^{-1}$).
