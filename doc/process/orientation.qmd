...
for conf in confnames:
    if os.path.isdir(os.path.join('ichthyop_output', conf)):
        ...
        if conf == 'reef':
            command = ['java', '-jar', jarname, os.path.join('ichthyop_configs', conf, f'{conf}-noadv.xml')]
            print(command)
            result = subprocess.run(command, capture_output=False, text=False)
```
...
#|echo: false
#|output: true
#|label: fig-velocity-pld
#|fig-cap: "Particle velocity as a function of age"
import matplotlib.pyplot as plt
import numpy as np
...
#|echo: false
#|output: true
#|label: fig-ref-vonmises
#|fig-cap: "Example of Von Mises distributions"
import matplotlib.pyplot as plt
import numpy as np
from scipy.special import i0
...
```{python}
#|label: fig-card
#|fig-cap: "Trajectory with cardinal orientation"
#|echo: false
#|output: true
import xarray as xr
import cartopy.crs as ccrs
...
#|output: true
#|label: fig-ref-rheo
#|fig-cap: "Trajectory with rheotaxis orientation"
import xarray as xr
import cartopy.crs as ccrs
import cartopy.feature as cfeature
import os
...
#|fig-cap: "Scheme representing the trajectory angle calculation"
import numpy as np
import matplotlib.pyplot as plt
...
mesh = xr.open_dataset('../input/nemo-indian_mesh_hgr.nc', decode_times=False)
lont = mesh['glamt'].squeeze().values
latt = mesh['gphit'].squeeze().values
mesh

lont = np.linspace(lont.min(), lont.max(), 200)
latt = np.linspace(latt.min(), latt.max(), 200)
lont, latt = np.meshgrid(lont, latt)

import cartopy.geodesic
g = cartopy.geodesic.Geodesic()
def compute_dist_bary(xp, yp):
    output = []
    for lontemp, lattemp in zip(np.ravel(lont), np.ravel(latt)):
        test = g.inverse([lontemp, lattemp], [xp, yp])
        output.append(test[0][0])
    output = np.reshape(output, lont.shape)
    return output

def closestPoint(lineStartA, lineEndB, thePoint):
        ...
        if (proj <= 0.0):
            ...
            #// square
            vsq = np.dot(tempLineEndB, tempLineEndB);
            #// B^2
            proj /= vsq;
...
def compute_dist_edges(xp, yp):
    ...
    for lontemp, lattemp in zip(np.ravel(lont), np.ravel(latt)):
        ...
        xyCoords = [lontemp, lattemp]

        for i in range(nEdges):

            start = xReefs[i], yReefs[i]
            end =  xReefs[i + 1], yReefs[i + 1]
            temp_closest = closestPoint(start, end, xyCoords);

            dist = np.sqrt(np.power(temp_closest[0] - xyCoords[0], 2) + np.power(temp_closest[1] - xyCoords[1], 2));

            if (dist <= minDistance):
                final_closest = temp_closest.copy();
                minDistance = dist;
        test = g.inverse([lontemp, lattemp], [final_closest[0], final_closest[1]])
        output.append(test[0][0])
...
def plot_figure(config):
    ...
    lon = lon.where(mort == 0)
    mortsum = mort.cumsum(dim='time')
    lon = lon.where(mortsum == 0)
    ...
    drifters = slice(None, None)

    projin = ccrs.PlateCarree()
    projout = ccrs.PlateCarree()
    itime = slice(None, None)

    D = 6e4
    plt.rcParams['lines.linewidth'] = 2

    # t2d = mort.values[itime, drifters]
    ax = plt.axes(projection = projout)
    ax.scatter(lon[itime, drifters], lat[itime, drifters], c=t2d[itime, drifters], marker='.', transform=projin, s=0.5, cmap=mpl.colormaps['jet'])
    ...
    xp = np.array([lon1, lon2, lon3, lon4, lon1])
    yp = np.array([lat1, lat2, lat3, lat4, lat1])
    xb = np.mean(xp[:4])
    yb = np.mean(yp[:4])

    dista = compute_dist_bary(xb, yb)
    # ax.plot(xb, yb, marker='o', color='r')
    ax.plot(xp, yp, transform=projin, color='r', ls='--')

    dista_edges = compute_dist_edges(xp, yp)

    lon1 = 51.625
    lat1 = -11.79877769947052
    lon2 = 52.625
    lat2 = -11.79877769947052
    lon3 = 52.625
    lat3 = -10.79877769947052
    lon4 = 51.625
    lat4 = -10.79877769947052
    ...
    xb = np.mean(xp[:4])
    yb = np.mean(yp[:4])
    distb = compute_dist_bary(xb, yb)
    distb_edges = compute_dist_edges(xp, yp)

    distot = np.min(np.array([dista, distb]), axis=0)
    distot_edges = np.min(np.array([dista_edges, distb_edges]), axis=0)

    ax.plot(xp, yp, transform=projin, color='b', ls='--')
    # ax.plot(xb, yb, marker='o', color='b')

    ax.set_extent([49.5 + 0.1, 55.25-1, -13.13 + 0.4, -9.45 - 0.5], crs=projin)

    distot = np.ma.masked_where(distot >= D, distot)
    distot_edges = np.ma.masked_where(distot_edges >= D, distot_edges)
    # distot_edges = np.ma.masked_where(mask == 0, distot_edges)

    distot_edges = 1 - distot_edges / D
    cs = ax.pcolormesh(lont, latt, distot_edges, transform=projin, zorder=0, alpha=0.3, cmap=mpl.colormaps['Spectral_r'])
```
...
#|output: true
#|label: fig-ref-reef
#|fig-cap: "Trajectory with reef orientation and advection. The cells where the reefs are detected are shown in colors"
...
#|echo: false
#|output: true
#|label: fig-ref-reef-noadv
#|fig-cap: "Trajectory with reef orientation only. The cells where the reefs are detected are shown in colors"

plot_figure('reef-noadv')
```