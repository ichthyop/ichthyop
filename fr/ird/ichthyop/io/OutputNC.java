 ...
 * @author P.Verley
 */
public class OutputNC implements ISimulationAccessor {
...
///////////////////////////////
     ...
     */
    private static ArrayChar.D2 arrDeath;
    /**
...
////////////////////////////
    private String getParameter(String key) {
        return getSimulation().getOutputManager().getParameter(key);
    }
     ...
     * the file.
     */
    public void create(int i_simu, int nb_simu, int typeSimu) throws
            IOException {

        String strNcOut;
        String pathOut = getParameter("directory_out");

        /** Ensures ouput path not null. Throws an exception otherwise */
        if (pathOut == null) {
            throw new IOException("Ouput path is null");
        }
        ...
        if (typeSimu == Constant.SERIAL) {
            ...
            strNcOut = pathOut + getParameter("output_filename") + "_";
            ...
            strNcOut = pathOut + getParameter("output_filename") + ".nc";
        }
        ...
        if (getSimulation().getActionManager().getXAction("action.recruitment").isEnabled()) {
            zone = ncOut.addDimension("recruit_zone", getSimulation().getZoneManager().getZones(TypeZone.RECRUITMENT).size());
        }
        ...
        if (Boolean.valueOf(getSimulation().getParameterManager().getValue("app.transport", "three_dimension"))) {
            addVar2NcOut(Field.TEMPERATURE);
            addVar2NcOut(Field.SALINITY);
        }
        if (getSimulation().getActionManager().getXAction("action.growth").isEnabled()) {
            addVar2NcOut(Field.LENGTH);
        }
        if (getSimulation().getZoneManager().getZones(TypeZone.RELEASE) != null) {
    ...
    private static void addGlobalAttribute2NcOut() {
        ...
        Configuration.getTypeModel() == Constant.ROMS ? "roms" : "mars");
        ncOut.addGlobalAttribute("scheme",
        Configuration.getScheme() == Constant.EULER ? "euler" : "rk4");
        ...
        String.valueOf((float) Configuration.getDurationInRecruitArea() / (float) Constant.ONE_DAY));
    ...
    private void init() {
        ...
        FLAG_GROWTH = getSimulation().getActionManager().getXAction("action.growth").isEnabled();
        FLAG_RECRUITMENT = getSimulation().getActionManager().getXAction("action.recruitment").isEnabled();
        FLAG_RELEASED_ZONE = getSimulation().getZoneManager().getZones(TypeZone.RELEASE) != null;
        nbRecruitmentZones = getSimulation().getZoneManager().getZones(TypeZone.RECRUITMENT).size();

        i_record = 0;
        int nbParticles = getSimulation().getReleaseManager().getNbParticles();
        arrLon = new ArrayFloat.D2(1, nbParticles);
        arrLat = new ArrayFloat.D2(1, nbParticles);
        arrDepth = new ArrayFloat.D2(1, nbParticles);
        arrTime = new ArrayDouble.D1(1);
        arrDeath = new ArrayChar.D2(1, nbParticles);
        if (FLAG_3D) {
            arrTp = new ArrayFloat.D2(1, nbParticles);
            arrSal = new ArrayFloat.D2(1, nbParticles);
        }
        if (FLAG_RELEASED_ZONE) {
            arrZone = new ArrayInt.D2(1, nbParticles);
        }
        if (FLAG_RECRUITMENT) {
            arrRecruit = new ArrayInt.D3(1, nbParticles,
                    nbRecruitmentZones);
        }
        if (FLAG_GROWTH) {
            arrLength = new ArrayFloat.D2(1, nbParticles);
        }
    ...
    public void write(double time) {
        ...
        System.out.println("  --> record " + i_record + " - time " +
                (long) time);
        Iv2Particle particle;
        Iterator<Iv2Particle> iter = getSimulation().getPopulation().iterator();
        ...
        while (iter.hasNext()) {
            ...
            //arrDeath.setString(i, particle.getDeathCause());
            if (FLAG_3D) {
            ...
            if (FLAG_RELEASED_ZONE) {
                arrZone.set(0, i, particle.getNumZone(TypeZone.RELEASE));
            }
        }
        arrTime.set(0, time);
    ...
    public enum Field {
        ...
        TIME("time", "time in second since origin", "second", DataType.DOUBLE, 1),
