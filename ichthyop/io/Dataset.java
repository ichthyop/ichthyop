...
//////////////////////////////////////
     ...
     */
    abstract void readConstantField(String gridFile) throws IOException;
     ...
     */
    abstract void readLonLat(String gridFile) throws IOException;
...
////////////////////////////
    ...
    public void init() throws IOException {
        ...
        boolean cache = Configuration.getCache() != null;

        if (cache) {
            cleanUpCache(filterInputList(listInputFiles));
        }
    ...
    private ArrayList<String> filterInputList(ArrayList<String> inputList) throws IOException {

        long t0 = Simulation.get_t0();
        long tf = t0 + Configuration.getTransportDuration();
        String first = getFile(t0);
        String last = getFile(tf);
        int ifirst = inputList.indexOf(first);
        int ilast = inputList.indexOf(last);

        ArrayList<String> filteredList = new ArrayList(ilast - ifirst + 1);
    ...
    private void cleanUpCache(ArrayList<String> inputList) throws IOException {

        // Get the basename list
        ArrayList<String> inputFilename = new ArrayList(inputList.size());
        ...
        // Clean up the cache
        File cache = new File(Configuration.getCache());
        String fileMask = Configuration.getFileMask();
        File[] listFile = cache.listFiles(new MetaFilenameFilter(fileMask));
        if (listFile != null) {
            for (File file : listFile) {
                if (!inputFilename.contains(file.getName())) {
                    file.delete();
                    System.out.println("Deleted from cache: " + file.getName());
                }
            }
        }
    }

    private String cache(File src) throws IOException {

        String path = Configuration.getCache();
        File dest = new File(path.concat(src.getName()));
        if (!dest.exists()) {
            copyFile(src, dest);
            System.out.println("Added to cache: " + src.getName());
        }
    ...
    private void openLocation(String rawPath) throws IOException {
        ...
        if (isDODS(rawPath)) {
            ...
            if (Configuration.isGridFile()) {
                gridFile = getGridFile(Configuration.getPathGridFile());
            } else {
        ...
        boolean cache = Configuration.getCache() != null;
        if (cache) {
            gridFile = cache(new File(gridFile));
        }
        open(getFile(Configuration.get_t0(0)));
    }
    ...
    private void open(String filename) throws IOException {
        ...
        String cachedfile = cache
                ? cache(new File(filename))
                : filename;

        try {
            if (ncIn == null ||
                    (new File(ncIn.getLocation()).compareTo(new File(cachedfile)) !=
                    0)) {
                System.out.println(Resources.MSG_OPEN + cachedfile);
                ncIn = NetcdfDataset.openFile(cachedfile, null);
                nbTimeRecords = ncIn.findDimension(strTimeDim).getLength();
            }
            System.out.print("Open dataset " + cachedfile + "\n");
        } catch (IOException e) {
            throw new IOException("Problem opening dataset " + cachedfile + " - " + e.getMessage());
        } catch (NullPointerException e) {
            throw new IOException("Problem reading " + strTimeDim + " dimension at location " + cachedfile +
                    " : " + e.getMessage());
        }
    }
