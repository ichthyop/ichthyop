...
///////////////////////////////
    ...
    private static Boolean BLN_GRIDFILE;
    private static String PATH_GRIDFILE;
...
////////////////////////////
    ...
    private boolean isV213(INIFile file) {
     ...
     * @return
     */
    private boolean isV212(INIFile file) {
     ...
     * @return
     */
    private boolean isV211(INIFile file) {

        String field = file.getStringProperty(Structure.SECTION_SIMULATION,
                Structure.VERSION);
    ...
    private INIFile upgrade(INIFile file, int version) throws IOException {
        ...
        if (version < 213) {
            ufile = u212To213(ufile);
        }

        try {
            if (getVersion(ufile) == VERSION) {
                ufile.save();
                System.out.println("File " + new File(ufile.getFileName()).getName() + " upgraded to " + versionToString(VERSION));
            }
    ...
    private INIFile u200To210(INIFile file) {
        ...
        System.out.println("up from 200 to 210");
    ...
    private INIFile u210To211(INIFile file) {
        ...
        System.out.println("up from 210 to 211");

        return ufile;
    }

    private INIFile u211To212(INIFile file) {

        INIFile ufile = file;

        // Add property [IO] <gridfile>
        ufile.setBooleanProperty(Structure.SECTION_IO,
                Structure.GRIDFILE,
                false,
                null);

        // Add property [IO] <path gridfile>
        ufile.setStringProperty(Structure.SECTION_IO,
                Structure.PATH_GRIDFILE,
                "null",
                null);

        // Update property [SIMULATION] <version>
        ufile.setIntegerProperty(Structure.SECTION_SIMULATION,
                Structure.VERSION,
                212,
                null);
        ...
        System.out.println("up from 211 to 212");

        return ufile;
    }

    private INIFile u212To213(INIFile file) {

        INIFile ufile = file;

        // Add property [IO] <cache>
        ufile.setStringProperty(Structure.SECTION_IO,
                Structure.CACHE,
                "null",
                null);
        ...
        System.out.println("up from 212 to 213");
    ...
    private void readIO(INIFile file, String section) {
        ...
        CACHE_PATH = readString(file, section, Structure.CACHE);
        FILE_MASK = readString(file, section, Structure.FILTER);
        BLN_GRIDFILE = file.getBooleanProperty(section, Structure.GRIDFILE);
        if (BLN_GRIDFILE) {
            PATH_GRIDFILE = readString(file, section, Structure.PATH_GRIDFILE);
        }
...
//////////
    ...
    //---------------------------------------------------------
    public static boolean isGridFile() {
        return BLN_GRIDFILE;
    }
    ...
    //---------------------------------------------------------
    public static String getPathGridFile() {
        return PATH_GRIDFILE;
    }
